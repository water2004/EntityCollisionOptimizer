...
- 预发布版本：`1.0.0-mc26.3-alpha.1`，对应 tag `v1.0.0-mc26.3-alpha.1`
- 正式版本：`1.0.0+mc26.3`，对应 tag `v1.0.0+mc26.3`

其中前三段是模组语义版本，`mc26.3` 必须与 `gradle.properties` 中的 `minecraft_version` 一致；预发布序号必须是不带前导零的非负整数。Git tag 在版本号前添加 `v`，JAR 文件名和 `fabric.mod.json` 中的版本不含 `v`。
...
git switch main
git pull --ff-only
git tag v1.0.0-mc26.3-alpha.1
git push origin v1.0.0-mc26.3-alpha.1
```
