...
另一组僵尸压力测试逐步增加实体数量，并以每秒五次的频率读取游戏内 HUD。下图实线是每 25 个实体区间的 MSPT 中位数，淡色点是原始读数，阴影表示四分位区间。关闭 ECO 时，服务器在约 924 个实体处越过 50 MSPT 的 tick 预算；两组开启 ECO 的记录在约 1,500 个实体时仍低于该限制。在此负载下，启用 `vanillaOrder` 只产生了很小的性能差异。

![关闭 ECO、开启 ECO 以及开启 ECO 和 vanillaOrder 时 MSPT 随实体数量的变化曲线](docs/images/comparison/mspt-vs-entity.png)
