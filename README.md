...
- 移动碰撞从 `Entity.collide`、两个 `collideBoundingBox` 及 `Entity.move` 的求解调用接管。native 用完整体素网格执行轴裁剪、台阶高度选择和目标位置计算，不执行 Lithium 对应的移动求解路径。Java 保留动态上下文形状收集及原版位置提交/世界回调顺序；重力、摩擦、碰撞后速度响应和方块效果还未全部迁入 native。不可变形状几何按对象缓存，查询只传有序引用和平移；原生缓冲按线程支持嵌套复用，不跨实体缓存上下文的形状选择。此功能同样由 `enableEntityCollision` 控制，独立于 Carpet 对实体推动数量的管理。
