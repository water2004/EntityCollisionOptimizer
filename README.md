...
- 普通实体推动按连续的原版行为分段执行 native 推动，包括普通僵尸继承的 `LivingEntity.push`。每段读取实时位置、速度和版本失效的推动状态，由 native 按原版顺序计算冲量、逐次累加速度并处理有限值和同步标志，段结束时写回真实实体字段。不会跨段或跨 tick 保留另一份待兑现速度，也不依赖 getter 物化。特殊推动前必先完成写回，并在后续普通段重新读取状态。有特殊推动或速度访问覆写的原版实体仍执行各自原版逻辑；这是行为分派，不是密度分流或失败回退。实现与测量见 [推动段执行记录](docs/native-push-run-2026-09-07.md)。
...
推动段执行另有 90 组 2/8/20 实体用例，按 double 原始位比较完整段和拆分段在写回边界的速度、同步标志；覆盖负零、大数舍入、非有限原始速度的拒绝行为、睡眠、`noPhysics` 和收集候选后来源死亡。另在真实船推动回调入口观察 Java 速度字段，验证“普通段—特殊推动—普通段”的写回顺序。测试共注册 21 项（含原版 `always_pass` 和显式启用的压测入口），不是 21 组压力实验。
